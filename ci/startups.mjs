...
const organisationsIds = (await fs.readdir(organisationsPath)).filter((path) => path.endsWith(".md")).map((p) => p.replace(/\.md$/, ""));

export const schema = z.object({
  title: z.string(),
  mission: z.string(),
  sponsors: z.array(z.enum(organisationsIds.map(id => `/organisations/${id}`))).optional(),
  incubator: z.string(),
  contact: z.string(),
  ...
  if (obj.phases.length !== Array.from(new Set(obj.phases.map(p=>p.name))).length) {
    ctx.addIssue({
